...
let get_index index (x0, x1, x2) =
  match index with Zero -> x0 | One -> x1 | Two -> x2
...
let is_full_row row =
  match row with Some _, Some _, Some _ -> true | _ -> false
...
let show_field = function None -> " " | Some p -> show_player p
