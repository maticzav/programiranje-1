...
let test_matrix = [| [| 1; 2; 0 |]; [| 2; 4; 5 |]; [| 7; 0; 1 |] |]
...
let path matrix =
  ...
  (* x, y merimo od zgoraj navzdol po y in levo proti desno po x*)
  let rec aux (x, y) =
    let new_x, new_y = (x + 1, y + 1) in
    (* Izračunaj optimalno pot glede na to kje v tabeli si. *)
    if new_x > width || new_y > height then (0, [])
    else
      (* Pogledaš obe poti. *)
      let sum_right, path_right = aux (new_x, y) in
      let sum_down, path_down = aux (x, new_y) in
      (* Vrneš večjo, če sta enaki greš desno. *)
      if sum_right >= sum_down then
        (matrix.(y).(x) + sum_right, Right :: path_right)
      else (matrix.(y).(x) + sum_down, Down :: path_down)
  in
  aux (0, 0)

let max_cheese matrix =
  let sum, path = path matrix in
  sum
...
let convert_path matrix path =
  let rec aux (x, y) = function
    | [] -> []
    | Down :: xs -> matrix.(y).(x) :: aux (x, y + 1) xs
    | Right :: xs -> matrix.(y).(x) :: aux (x + 1, y) xs
  in
  aux (0, 0) path
