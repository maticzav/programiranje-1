...
def najdaljse_narascajoce_zaporedje(zaporedje):
    ...
    def podzaporedje(i, zadnji):
        ...
        nov_clen = zaporedje[i]
        if nov_clen >= zadnji:
            # Vsak člen lahko ali dodamo ali ne dodamo.
            vzamemo = [nov_clen] + podzaporedje(i + 1, nov_clen)
            ...
            # Gremo naprej ali žačnemo novo zaporedje 
            # ([nov_clen], ki pa je zagotovo manjše kot zaporedje brez tega, 
            #  saj nam ga funkcija od prej že pobere).
...
def zabica(mocvara):
    ...
    @lru_cache(maxsize=None)
    def skakanje(energija, i):
        # Če smo zunaj ne rabimo več skakati.
        if i >= k:
            return 0, []
        ...
        # Pogledamo vse možnosti na katere lahko skočimo in vrnemo
        # tisto pri kateri v najmanj skokih pridemo ven.
        poti = [(skakanje(nova_energija - skok, i + skok), skok) for skok in range(1, nova_energija + 1)]
        (dolzina, pot), skok = min(poti, key=lambda x: x[0])
        return dolzina + 1, [skok] + pot
